...
% links a geographical location to the corresponding text and generates a footnote with the link
...
% register a new link, display it in the text and generate a footnote. the label in the text can be set by the optional first argument. if the first argument is not given, the whole link address gets displayed in the text
...
{%
	\iffirstargument
		\RegTransLa{{\ss#2}\bracket{#1}}
		\color[magenta:8]{\em#2}\bracket{#1}
	\else
		\RegTransLaUdef{{\ss#1} <> translation missing}
...
%\define[1]\aDefinition{\regDefinition{#1}}
