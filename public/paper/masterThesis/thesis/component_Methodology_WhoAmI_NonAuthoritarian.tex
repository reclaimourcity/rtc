...
\define[]\subsectionNonAuthoritarian{\subsection[methodology::whoami::nonauthoritarian]{Non-Authoritarian//Non-Hierarchical}}
...
Now, this thesis exceeds those specifications due to the fact that research action(s) in São Paulo already lasted 6 month. Thus, once more, one enters the question of constraints and benefits for the research agent, which probably wouldn't allow further reflection and adjustments of the thesis context due to its strict time setting and would have probably led to a work that just follows the logic of acquiring a title or developing a technical fix \refMissing{tyranny - participation} while leaving approach, outcome and effect of conducted research actions rather insignificant, just as necessary means to the anticipated end.

\spaceHalf

\inright{two antithetic poles: approaching practice}
It also questions the conditions under which research actions could be realized and how research is organized: either embedded in existing institutional frameworks \bracket{such as social or technical focused university programs or NGO \refMissingSrc{what is an NGO link missing} projects to mention just a few} and by that following the logic of a projects inherent constraints \bracket{i.e. fulfilment of fixed projustjects milestones or limited time frames, to name just a few}, or self-determined, transparent, emancipatory and reflective over the course of actions. 

\spaceHalf

\inright{participation as one component of the thesis and its research action(s)}
Those two notions \bracket{that may represent two antithetic poles, out of many} are closely related to different denotation of \quote{participation}, which can be claimed to be inherent in both antithetic approaches. The concept of \quote{\aKeyword{participation}} is apparent in the thesis title but also in the realized research actions in São Paulo. Therefore, an in depth conceptualization forms one component of this thesis \refMissingSrc{theory participation} but forms as well one component of the concrete research action(s), lain out below \refMissingSrc{methodology participation and participant observation}.

\startARemark
hier könnte nochmal eine übersicht hin, welche themen von der antiathoritären frage durchzogen sind, als letzter überblick bevor wissenschaftliche herangehensweisen betrachtet werden.
\stopARemark

\textBoxedRoundMax{final overview of mentioned anti-authoritarian / non-hierarchical themes.}
