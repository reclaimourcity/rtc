...
{%
	\iffirstargument
		...
		\RegLocation{#1: \goto{\url[#2]}[url(#2)]}
	\else
		\useURL[#2][#2][][#2]\from[#2]
		\footnote[#2]{location: \goto{\url[#2]}[url(#2)]}
		\RegLocation{\goto{\url[#2]}[url(#2)]}
	\fi
}
...
{%
	\iffirstargument
		\useURL[#2][#2][][#1]\from[#2]
		\RegMedia{#1: \goto{\hyphenatedurl{#2}}[url(#2)]}
		\footnote[#2]{#1: \goto{\hyphenatedurl{#2}}[url(#2)]}
	\else
		\useURL[#2][#2][][#2]\from[#2]
		\RegMedia{media: \goto{\hyphenatedurl{#2}}[url(#2)]}
		\footnote[#2]{media: \goto{\hyphenatedurl{#2}}[url(#2)]}
	\fi
}

% for registering links of (mass) media sources
\def\aLinkSourceNew{\dosingleempty\doALinkSourceNew}
\def\doALinkSourceNew[#1]#2%
{%
	\iffirstargument
		\useURL[#2][#2][][#1]\from[#2]
		\RegSource{#1: \goto{\hyphenatedurl{#2}}[url(#2)]}
		\footnote[#2]{\goto{\hyphenatedurl{#2}}[url(#2)]}
	\else
		\useURL[#2][#2][][#2]\from[#2]
		\RegSource{source: \goto{\hyphenatedurl{#2}}[url(#2)]}
		\footnote[#2]{\goto{\hyphenatedurl{#2}}[url(#2)]}
	\fi
}

\def\aLinkSourceNewInVi{\dosingleempty\doALinkSourceNewInVi}
\def\doALinkSourceNewInVi[#1]#2%
{%
	\iffirstargument
		\useURL[#2][#2][][#1]
		\RegSource{#1: \goto{\hyphenatedurl{#2}}[url(#2)]}
...
{%
	\iffirstargument
		\useURL[#2][#2][][#1]\from[#2]
		\RegLink{#1: \goto{\hyphenatedurl{#2}}[url(#2)]}
		\footnote[#2]{#1: \goto{\hyphenatedurl{#2}}[url(#2)]}
	\else
		\useURL[#2][#2][][#2]\from[#2]
		\RegLink{\goto{\hyphenatedurl{#2}}[url(#2)]}
...
{%
	\iffirstargument
		\useURL[#3][#3][][#1]\from[#3]
		\RegLink{\goto{\hyphenatedurl{#2}}[url(#3)]}
	\else
		\useURL[#3][#3][][#2]\from[#3]
		\RegLink{\goto{\hyphenatedurl{#2}}[url(#3)]}
	\fi
...
\define[1]\addMedia{\RegReferenceMedia{#1}}
\define[1]\addSource{\RegReferenceSource{#1}}
...
\define[]\showReferencesSource{\placeRefs{\placeRegReferenceSource}}

\define[]\showLinks{\placeRefs{\placeRegLink}}
\define[]\showLinksSources{\placeRefs{\placeRegSource}}
\define[]\showLinksMedias{\placeRefs{\placeRegMedia}}
\define[]\showLinksLocations{\placeRefs{\placeRegLocation}}

\define[]\showReferencesAll{\placeRefs{\placeRegReference[criterium=all]}}
\define[]\showReferencesMediaAll{\placeRefs{\placeRegReferenceMedia[criterium=all]}}
\define[]\showReferencesSourceAll{\placeRefs{\placeRegReferenceSource[criterium=all]}}

\define[]\showLinksAll{\placeRefs{\placeRegLink[criterium=all]}}
\define[]\showLinksSourcesAll{\placeRefs{\placeRegSource[criterium=all]}}
\define[]\showLinksMediasAll{\placeRefs{\placeRegMedia[criterium=all]}}
\define[]\showLinksLocationsAll{\placeRefs{\placeRegLocation[criterium=all]}}
...
\define[]\startDialog%
{%
%\startcolor[magenta:8]\startlines
\setupTABLE[r][each][foregroundcolor=color, foregroundcolor=magenta:8, frame=off]
\setupTABLE[c][1][width=1cm]
\setupTABLE[c][2][style=\tt]
\setupTABLE[c][3][style=\tt\em]
\spaceHalf
\bTABLE[split=yes]
}
